...
class QmcBlockedDense(kfac_jax.TwoKroneckerFactored):
    ...
    def _init(
        ...
        for power in approx_powers_to_cache:
            if power != -1:
                raise NotImplementedError(
                    f"Approximations for power {power} is not " f"yet implemented."
                )
        ...
        return kfac_jax.TwoKroneckerFactored.State(
            ...
            outputs_factor=kfac_jax.utils.WeightedMovingAverage.zero((j, m * n, m * n)),
        )
    ...
    def _update_cache(
        ...
        for power in approx_powers:
            ...
            cache["inputs_factor"], cache["outputs_factor"] = pi_adjusted_inverse(
                state.inputs_factor.value, state.outputs_factor.value, identity_weight
            )
        return state
